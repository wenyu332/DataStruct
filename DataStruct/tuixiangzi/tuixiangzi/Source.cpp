...
void xinxi()
{
	...
	for (i = 0; i<N; i++)
	{
		...
		scanf_s("%s", bk[i].name);
		printf_s("\t\t请输入第%d本书的出版社:", i + 1);
		scanf_s("%s", bk[i].publish);
		printf("\t\t请输入第%d本书的出版时间(用逗号隔开):", i + 1);
		scanf_s("%d,%d,%d", &bk[i].t.year, &bk[i].t.month, &bk[i].t.day);
...
void show()
{
	...
	do
	{
		...
		switch (x)
		{
		case 1:system("cls"); showbook_name(); _getch(); break;/*按书名查看*/
		case 2:system("cls"); showbook_num(); _getch(); break;/*按书号查看*/
		case 3:system("cls"); weijieyue(); _getch(); break;/*查看未借阅图书*/
		case 4:system("cls"); yijieyue(); _getch(); break;/*查看已借阅图书*/
		case 5:system("cls"); show_all_book(); _getch(); break;/*查看所有图书*/
		}
	} while (x != 6);
}
/*********按书名借阅*******/
...
void jieyue()
{
	...
	do
	{
		...
		switch (x)
		{
		case 1:system("CLS"); jie_name(); _getch(); break;/*按书名借阅*/
		case 2:system("CLS"); jie_num(); _getch(); break;/*按书号借阅*/
		case 3:system("cls"); show_all_book(); _getch(); break;/*查看所有图书*/
		}
	} while (x != 4);
}
/***************按书名进行查找并修改信息*************/
		...
		else
		{
			for (i = 0; i<N; i++)
				if (strcmp(temp, bk[i].name) == 0)
				{
					...
					printf("\t\t现在请输入新信息:\n");
					printf("\t\t请输入本书的编号:");
					scanf_s("%d", &bk[book_xb].num);
					printf("\t\t请输入本书的名称:");
					scanf_s("%s", bk[book_xb].name);
					printf("\t\t请输入本书的出版社:");
					scanf_s("%s", bk[book_xb].publish);
					printf("\t\t请输入本书的出版时间(用逗号隔开):");
					scanf_s("%d,%d,%d", &bk[book_xb].t.year, &bk[book_xb].t.month, &bk[book_xb].t.day);
					printf("\t\t该书是否已经被借阅,已被借阅输入'1'，未被借阅输入'0':");
					scanf_s("%d", &bk[book_xb].jie);
				}
			if (k == 0)
			{
			...
			printf("\t恭喜!信息修改成功!任意键返回..."); _getch(); break;
		}
	}
}
/***************按书号进行查找并修改信息*************/
...
void dele_name()
{
	...
	if (strcmp(temp_name, "0") != 0)
	{
		...
		if (k == 0)
			printf("\t未找到该书,请核实以后再操作!,按任意键返回...."); _getch();
	}
}
/***************按书号查找并删除***********/
void dele_num()
{
	...
	while (1)
	{
		...
		else
		{
			...
			if (k == 0)
				printf("\t未找到该书,请核实以后再操作!,按任意键返回...."); _getch(); break;
		}
	}
}
void dele()
{
	...
	do
	{
		...
		case 3:system("cls"); dele_all(); _getch(); break;
		}
	} while (x != 4);
}
	...
	do
	{
		...
		case 1:/************调用管理员模式函数***********/
			...
			do
			{
				if (s == 1)
				{
					...
					switch (x2)
					{
					case 1:system("cls"); xinxi(); _getch(); break;/*录入信息*/
		...
		case 2:/*调用客户模式函数*/
			do
			{
				...
				switch (x2)
				{
				case 1:system("cls"); show(); _getch(); break;/*查看图书信息*/
				case 2:system("cls"); jieyue(); _getch(); break;/*借阅图书*/
				}
			} while (x2 != 3);
		}
	} while (x != 3);
